...
class RandomForestClassifier:
    ...
    def fit(self, X, y, categorical):
        ...
        for tree in self.decision_trees:
            ...
            bootstrap = np.random.choice(np.arange(X[:,0].size), size = int(.8 * X[:,0].size), replace = False)  
            
            # Generate a feature subset
            features = np.random.choice(np.arange(X[0].size), size = int(.8 * X[0].size), replace = False)
