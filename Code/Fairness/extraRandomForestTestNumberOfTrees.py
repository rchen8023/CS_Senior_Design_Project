...
from extraRandomForestWithRidgeRegression import FairExtraRandomForestClassifier
...
model = FairExtraRandomForestClassifier(max_depth = 23, num_trees = 500)

num_tree_size = 10
num_trial = 10

# Setup arrays
error_train = np.zeros((num_trial, num_tree_size))
error_test = np.zeros((num_trial, num_tree_size))

probability_difference_test1 =np.zeros((num_trial, num_tree_size))
probability_difference_test2 =np.zeros((num_trial, num_tree_size))

demographic_parity_test1 = np.zeros((num_trial, num_tree_size))
demographic_parity_test2 = np.zeros((num_trial, num_tree_size))

# intersectional
probability_difference_test3 =np.zeros((num_trial, num_tree_size))
probability_difference_test4 =np.zeros((num_trial, num_tree_size))

probability_difference_testa1 = np.zeros((num_trial,num_tree_size))
probability_difference_testa2 = np.zeros((num_trial,num_tree_size))
probability_difference_testa3 = np.zeros((num_trial,num_tree_size))
probability_difference_testa4 = np.zeros((num_trial,num_tree_size))

demographic_parity_test3 = np.zeros((num_trial, num_tree_size))
demographic_parity_test4 = np.zeros((num_trial, num_tree_size))

demographic_parity_testa1 = np.zeros((num_trial, num_tree_size))
demographic_parity_testa2 = np.zeros((num_trial, num_tree_size))
demographic_parity_testa3 = np.zeros((num_trial, num_tree_size))
demographic_parity_testa4 = np.zeros((num_trial, num_tree_size))

training_instances = np.zeros(num_tree_size)
...
for k in range(0, num_trial):
    ...
    for i in range(0, num_tree_size):
        ...
        # Increase the training size
        num_trees = 10-*(i+1)
        training_instances[i] = (num_trees)
        ...
        model.fit(sample_train,label_train, num_fair_trees = num_trees)
        ...
        # Calculate the probability we predict someone will dropout between groups (Demographic Parity)
        num_Mu_predict = len([e for e in range(0, num_test) if label_test_predicted[e] == 0 and sample_test[e, 0] == 0 and sample_test[e, 16] == 1])
        num_Wr_predict = len([e for e in range(0, num_test) if label_test_predicted[e] == 0 and sample_test[e, 0] == 1 and sample_test[e, 16] == 0])
        num_Wu_predict = len([e for e in range(0, num_test) if label_test_predicted[e] == 0 and sample_test[e, 0] == 1 and sample_test[e, 16] == 1])
        num_Mr_predict = len([e for e in range(0, num_test) if label_test_predicted[e] == 0 and sample_test[e, 0] == 0 and sample_test[e, 16] == 0])
